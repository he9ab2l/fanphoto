...
所有 JSON 接口位于 `/api` 前缀之下，契约定义在 `packages/contracts/src/index.ts`（唯一事实来源）。

较大的 JSON 响应按 `Accept-Encoding` 压缩；字段契约不变，且为 `no-store`。
媒体授权在 ETag 判断之前执行，撤回公开后的缓存请求同样无法绕过可见性检查。

错误响应统一为 `{ "error": { "code": "...", "message": "..." } }`，同时返回相应 HTTP 状态。
不要只判断 HTTP 200：上传新建为 201，重复导入为 200，校验失败为 400，未认证 401，越权 403，
...
| 方法   | 路径                 | 返回 / 用途                                                  |
| ------ | -------------------- | ------------------------------------------------------------ |
| GET    | /health              | `{ ok, version, storage }`；连接数据库做探活                 |
...
variant 为 sm / md / lg / original；元数据提供实际派生尺寸，用于 srcset。
媒体先验证公开 / 回收状态，再考虑条件缓存；命中 `If-None-Match` 返回 304，否则返回
ETag、`private, no-cache` 与内容。`original` 变体受下载开关限制（管理员不受限）。
...
| GET    | /admin/photos               | 同图库参数，另支持 status=all/public/private/trash                                                    |
...
| DELETE | /admin/photos/:id/permanent | 仅接受已在回收站的照片；永久删除文件与记录                                                            |
...
| PATCH  | /admin/settings             | `{ settings }`；保存站点设置                                                                          |
...
- `file`：原始照片文件（扩展名 / MIME 由文件头判定，限制 50 MB 源、8000 万像素）。
...
响应：新建返回 201，按源文件 SHA256 查重后的重复导入返回 200。处理为有界队列，处理器忙时
返回 `PROCESSOR_BUSY`（503，附 `Retry-After`）。
...
| SETUP_REQUIRED       | 尚未初始化环境（503，先运行 setup）               |
