...
自托管照片墙与摄影工作室。浏览器端提供照片墙展示与管理后台，Node 服务负责原片处理与派生图
生成，数据存于本地 SQLite，通过 `/api` 契约交互。
...
- 原文件保存在私有档案，公开图像不嵌入 EXIF；界面只展示从原片提取的真实信息。
...
- Node 24 LTS、pnpm 9。
- 图片处理依赖 sharp，需与运行平台匹配的原生二进制；arm64 平台建议使用对应构建或容器。
