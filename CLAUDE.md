...
- 当前发布：`release.cBYkTK`（2026-09-09，Glass Engine 六缺陷修复版，已合并 main 并切换 `current`）；44 项回归 + verify-live 域名验收通过
- 分支：只剩 `main`（feat/glass-engine 已合并删除）；发布后旧 release 与过渡备份已清理，只留最新一份
